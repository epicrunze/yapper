...
def calculate_rewards(prompts, completions, qa, **kwargs):
    ...
    for response in responses:
        ...
        # Sample one random question and its answer
        random_idx = random.randint(0, len(qas) - 1)
        sampled_question = qas[random_idx]["Question"]
        sampled_answer = qas[random_idx]["Answer"]
        
        # Calculate r_qa for the sampled question
        r_qa = calculate_qa_reward(response, sampled_question, sampled_answer)

        if r_qa:
            rewards.append(r_len)
        else:
            lambda_qa = -0.2
            rewards.append(lambda_qa)
