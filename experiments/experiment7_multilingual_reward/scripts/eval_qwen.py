...
os.environ["TRANSFORMERS_CACHE"] = "/work/kz133/llms/.cache"
os.environ["HF_HOME"] = "/work/kz133/llms/.cache"
...
parser.add_argument(
    "--results_base_path",
    type=str,
    default="/hpc/home/bfa6/work/github/yapper/experiments/experiment3_lambda_normal/results",
...
# Constants
max_seq_length = 2048
YAPPER_DIR = Path("/work/kz133/yapper")
...
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    cache_dir="/work/kz133/llms/.cache",
...
def get_multilingual_reward(text):
    ...
    for character in text:
        if prev_script is None:
            prev_script = unicodedataplus.script(character)
            curr_str = curr_str + character
        elif unicodedataplus.script(character) in [prev_script, "Common"]:
            curr_str = curr_str + character
        else:
            key = prev_script
            value = get_num_tokens(curr_str)
    ...
    counts = np.array(list(token_counts.values()), dtype=float)
...
for sample in tqdm(eval_data):
    ...
    # Calculate Length and Multilingual Rewards
    len_reward = get_length_reward(original_chunk, compressed_chunk)
    multilingual_reward = get_multilingual_reward(compressed_chunk)
    length_rewards.append(len_reward)
    multilingual_rewards.append(multilingual_reward)
    ...
    results.append({
        ...
        "multilingual_reward": multilingual_reward,
...
avg_multilingual_reward = np.mean(multilingual_rewards)
...
print(f"Average Multilingual Reward: {avg_multilingual_reward:.4f}")
...
with open(results_file, "w") as f:
    json.dump({
        ...
        "avg_multilingual_reward": avg_multilingual_reward,
...
# 1a. Histogram of Length Rewards
...
# 1b. Histogram of Multilingual Rewards
plt.figure(figsize=(10, 6))
sns.histplot(multilingual_rewards, bins=20, kde=True)
plt.title("Distribution of Length Rewards")
plt.xlabel("Length Reward")
plt.ylabel("Count")
plt.savefig(PLOTS_DIR / "multilingual_reward_hist.png")
plt.close()
...
# 3a. Scatter Plot: Length Reward vs QA Accuracy
plt.figure(figsize=(10, 6))
...
# 3b. Scatter Plot: Multilingual Reward vs QA Accuracy
plt.figure(figsize=(10, 6))
sns.scatterplot(x=multilingual_rewards, y=qa_accuracies)
plt.title("Length Reward vs QA Accuracy")
plt.xlabel("Length Reward")
plt.ylabel("QA Accuracy")
plt.savefig(PLOTS_DIR / "length_vs_qa_scatter.png")
